...
window.sendQtToWhatsapp = function() {
  ...
  let paymentLines = '';
  if (isFullPayment) {
    paymentLines = `💰 *Jumlah Bayaran Penuh*: RM${(currentQtData.total || 0).toLocaleString()} (100%)\n`;
  } else {
...
function renderHistoryItemsUI(history, container) {
  if (!history || history.length === 0) {
    ...
    container.innerHTML = `
      <div style="text-align: center; padding: 48px 20px; background: #f8fafc; border: 1.5px dashed #cbd5e1; border-radius: 16px; margin: 10px 0;">
        ...
        <h3 style="color: #0F172A !important; font-size: 17px; font-weight: 800; margin: 0 0 6px 0;">${isFiltered ? 'No Records Found' : 'No Documents Archived Yet'}</h3>
        <p style="color: #64748B !important; font-size: 13.5px; font-weight: 600; margin: 0;">${isFiltered ? 'Please try a different keyword or category filter.' : 'Generated quotations & invoices will automatically be archived here.'}</p>
      </div>
    `;
    return;
  }
  ...
  history.forEach((qt) => {
    ...
    const createdDate = new Date(qt.created_at || Date.now()).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
    ...
    html += `
      <div class="history-item-card" style="background: ${cardBg}; border: 1.5px solid ${isPinned ? '#FCD34D' : '#cbd5e1'}; border-top: ${cardTopBorder}; border-radius: 16px; padding: 16px; display: flex; flex-direction: column; justify-content: space-between; box-shadow: ${isPinned ? '0 6px 16px rgba(245,158,11,0.12)' : '0 4px 12px rgba(0,0,0,0.03)'}; position: relative;">
        <div>
          ...
          <div class="hic-header" style="display: flex; align-items: center; justify-content: space-between; gap: 6px; margin-bottom: 12px;">
            <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
              ...
              ${isPaid ? `<span style="font-size: 10.5px; background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; padding: 3px 8px; border-radius: 6px; font-weight: 800; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#047857" stroke-width="3"><polyline points="20 6 9 17 4 12"></polyline></svg> PAID</span>` : isSigned ? `<span style="font-size: 10.5px; background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; padding: 3px 8px; border-radius: 6px; font-weight: 800; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#047857" stroke-width="3"><polyline points="20 6 9 17 4 12"></polyline></svg> SIGNED</span>` : isInvoice ? `<span style="font-size: 10.5px; background: #dbeafe; color: #1e40af; border: 1px solid #93c5fd; padding: 3px 8px; border-radius: 6px; font-weight: 900; white-space: nowrap;">INVOICE</span>` : `<span style="font-size: 10.5px; background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; padding: 3px 8px; border-radius: 6px; font-weight: 700; white-space: nowrap;">QUOTATION</span>`}
          ...
          <h3 class="hic-title" style="font-size: 16px; font-weight: 800; color: #0f172a; margin: 0 0 4px 0; line-height: 1.3;">${escapeHtml(qt.client_name || qt.clientName || 'Client')}</h3>
          <p class="hic-sub" style="font-size: 12.5px; font-weight: 600; color: #475569; margin: 0 0 12px 0; line-height: 1.4;">${escapeHtml(qt.project_title || qt.projectTitle || 'Web Project')}</p>
          ...
          <div class="hic-price-row" style="background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 10px; padding: 9px 10px; margin-bottom: 14px; display: flex; justify-content: space-between; align-items: center; gap: 4px; font-size: 11.5px; font-weight: 700; color: #0f172a; white-space: nowrap !important; box-sizing: border-box;">
            <span style="white-space: nowrap !important;">Total: <strong style="color: #000000; font-weight: 900; font-size: 12px;">RM ${totVal}</strong></span>
            <span style="white-space: nowrap !important;">${isFullPayment ? 'Full' : 'Deposit'}: <strong style="color: ${isFullPayment ? '#1d4ed8' : '#047857'}; font-weight: 900; font-size: 12px;">RM ${depVal}</strong></span>
          </div>
        </div>
        ...
        <div class="hic-actions" style="display: flex; gap: 0px; align-items: center; justify-content: flex-start; border-top: 1px dashed #e2e8f0; padding-top: 10px;">
          <button class="btn-history-icon" onclick="viewHistoryQt('${qtNo}')" title="View Document" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#FFFFFF!important;border:1.5px solid #CBD5E1!important;color:#000!important;cursor:pointer!important;flex-shrink:0!important;">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
          </button>
          <button class="btn-history-icon" onclick="editHistoryQt('${qtNo}')" title="Edit Document" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#FFFFFF!important;border:1.5px solid #CBD5E1!important;color:#000!important;cursor:pointer!important;flex-shrink:0!important;">
          ...
          <button class="btn-history-icon" onclick="duplicateHistoryQt('${qtNo}')" title="Duplicate Document" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#FFFFFF!important;border:1.5px solid #CBD5E1!important;color:#0F172A!important;cursor:pointer!important;flex-shrink:0!important;">
          ...
          ${isInvoice
            ? `<a href="resit.html?rec=${encodeURIComponent(qtNo)}&name=${encodeURIComponent(qt.client_name || qt.clientName || '')}&amount=${qt.total || 0}&title=${encodeURIComponent(qt.project_title || qt.projectTitle || '')}" target="_blank" class="btn-history-icon" title="View Official Receipt" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#EFF6FF!important;border:1.5px solid #BFDBFE!important;color:#1D4ED8!important;cursor:pointer!important;flex-shrink:0!important;text-decoration:none!important;">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>
              </a>`
            : `<button class="btn-history-icon" onclick="convertQtToInvoice('${qtNo}')" title="Convert to Official Invoice" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#ECFDF5!important;border:1.5px solid #A7F3D0!important;color:#047857!important;cursor:pointer!important;flex-shrink:0!important;">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>
              </button>`
          }
          <button class="btn-history-icon" onclick="waHistoryQt('${qtNo}')" title="Share to WhatsApp" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#FFFFFF!important;border:1.5px solid #CBD5E1!important;cursor:pointer!important;flex-shrink:0!important;">
          ...
          <button class="btn-history-icon" onclick="deleteHistoryQt('${qtNo}')" title="Delete" style="width:32px!important;height:32px!important;min-width:32px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:#FFFFFF!important;border:1.5px solid #CBD5E1!important;color:#DC2626!important;cursor:pointer!important;flex-shrink:0!important;">
